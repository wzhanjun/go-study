...
type EchoClient interface {
	...
	BidirectionalStreamEcho(ctx context.Context, opts ...grpc.CallOption) (Echo_BidirectionalStreamEchoClient, error)
}
...
func (c *echoClient) BidirectionalStreamEcho(ctx context.Context, opts ...grpc.CallOption) (Echo_BidirectionalStreamEchoClient, error) {
	stream, err := c.cc.NewStream(ctx, &Echo_ServiceDesc.Streams[2], "/echo.Echo/BidirectionalStreamEcho", opts...)
	if err != nil {
		return nil, err
	}
	x := &echoBidirectionalStreamEchoClient{stream}
	return x, nil
}

type Echo_BidirectionalStreamEchoClient interface {
	Send(*EchoRequest) error
	Recv() (*EchoResponse, error)
	grpc.ClientStream
}

type echoBidirectionalStreamEchoClient struct {
	grpc.ClientStream
}

func (x *echoBidirectionalStreamEchoClient) Send(m *EchoRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *echoBidirectionalStreamEchoClient) Recv() (*EchoResponse, error) {
	m := new(EchoResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type EchoServer interface {
	...
	BidirectionalStreamEcho(Echo_BidirectionalStreamEchoServer) error
}
...
func (UnimplementedEchoServer) BidirectionalStreamEcho(Echo_BidirectionalStreamEchoServer) error {
...
func _Echo_BidirectionalStreamEcho_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(EchoServer).BidirectionalStreamEcho(&echoBidirectionalStreamEchoServer{stream})
}

type Echo_BidirectionalStreamEchoServer interface {
	Send(*EchoResponse) error
	Recv() (*EchoRequest, error)
	grpc.ServerStream
}

type echoBidirectionalStreamEchoServer struct {
	grpc.ServerStream
}

func (x *echoBidirectionalStreamEchoServer) Send(m *EchoResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *echoBidirectionalStreamEchoServer) Recv() (*EchoRequest, error) {
	m := new(EchoRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
